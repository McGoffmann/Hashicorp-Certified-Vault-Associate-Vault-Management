...
# Assuming we're in root of git repo at the moment
parentPath=$(pwd)
...
openssl x509 -req -days 180 -CA $certPath/ca.cert.pem -CAkey $certPath/ca.key.pem -CAcreateserial -in $certPath/server1.csr -out $certPath/server1.pem -extfile $certPath/server1.conf -extensions v3_req
...
docker run --name server1 --hostname server1 -d -v /${PWD}/server1:/vault --cap-add=IPC_LOCK -p 8200:8200 vault:1.13.2 server
